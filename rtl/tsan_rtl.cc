...
// 0 -> print all
static int _M_addr_len = 0;
void _M_set_addr_len(int a) {
  _M_addr_len = a;
}

static uptr _M_addrs[125];
void _M_set_addrs(uptr as[]) {
for(int i = 0; i < _M_addr_len; ++i) {
_M_addrs[i] = as[i];
}
}
...
void MemoryAccessImpl1(ThreadState *thr, uptr addr,
  ...
  Shadow old(0);
bool dprint = false;//_M_addr_len < 0;
...
void print_sw(ThreadState *thr, u64* ptr) {
  Shadow s(*ptr);
  Printf("addr0=%d  epoch=%d   tid=%d   w?=%d sz=%d   atm?=%d   f?=%d   HB?=%d\r\n",
         s.addr0(), s.epoch(), s.TidWithIgnore(), s.IsWrite(), s.size(), s.IsAtomic(), s.IsFreed(), HappensBefore(s, thr));
}
void print_sm(ThreadState *thr, u64* ptr) {
    int j = 0;
    for (; j < 4; ++j) {
        print_sw(thr, ptr + j);
    }
...
int kAccessSizeLog, bool kAccessIsWrite, bool kIsAtomic, u64 idx) {

//bool isTree2(ThreadState *thr, uptr pc, uptr addr, bool isWrite);
if(__tsan::aser::tree::isTree3(thr, pc, addr, kAccessIsWrite, idx)) {
return;
}
...
//    bool do_print = false;//_M_addr_len < 0;
//    for(int i = 0; i < _M_addr_len; ++i) {
//        if (_M_addrs[i] == addr) {
//          do_print = true;
//          break;
...
//    if (do_print) {
//        Printf("\r\n----------------   Current   ------------------\r\n#%d access @%p size=%d"
//                 " write?=%d shadow_mem=%p epoch=%d\n",
//         (int)thr->fast_state.tid(), (void*)addr,
//         (int)(1 << kAccessSizeLog), kAccessIsWrite, shadow_mem, thr->fast_state.epoch());
//        print_sm(thr, shadow_mem);
//    }
...
if(__tsan::aser::tree::isTree2(thr, pc, addr, kAccessIsWrite)) {
  return;
}

//Printf(">>> IN-TSAN Addr=%p  PC=%p\r\n", addr, pc);
